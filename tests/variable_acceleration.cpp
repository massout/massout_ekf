...
int main() {
    ...
    AccelerationFilter::StateMatrix covariance = AccelerationFilter::StateMatrix::Identity();
    covariance *= 25.0f;
    filter.setCovariance(covariance);

    const float dt = 1.0f;

    AccelerationFilter::StateMatrix transition = AccelerationFilter::StateMatrix::Identity();
    transition(0, 1) = dt;
    ...
    AccelerationFilter::StateMatrix process_noise = AccelerationFilter::StateMatrix::Identity();
    process_noise *= 0.05f;
    ...
    AccelerationFilter::MeasurementCovariance measurement_noise = AccelerationFilter::MeasurementCovariance::Identity();
    ...
    const size_t kSteps = sizeof(acceleration_sequence) / sizeof(acceleration_sequence[0]);
    ...
    if (absoluteValue(position_error) > 0.5f || absoluteValue(velocity_error) > 0.5f) {
        std::printf("Variable acceleration test failed: position=%f velocity=%f\n", position_error, velocity_error);

        return 1;
    }

    std::printf("Variable acceleration final estimate position=%f velocity=%f\n", estimate(0, 0), estimate(1, 0));
