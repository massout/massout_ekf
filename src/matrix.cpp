...
template <typename T, size_t Rows, size_t Cols>
Matrix<T, Rows, Cols> Matrix<T, Rows, Cols>::operator-(const Matrix& rhs) const {
    Matrix result;
...
Matrix<T, Rows, OtherCols> Matrix<T, Rows, Cols>::operator*(const Matrix<T, Cols, OtherCols>& rhs) const {
...
template <typename T, size_t Rows, size_t Cols>
bool Matrix<T, Rows, Cols>::tryCholeskyInverse(const Matrix& input, Matrix& out, T epsilon) {
    if (Rows != Cols || Rows == 0) {
        return false;
    }
...
template <typename T, size_t Rows, size_t Cols>
bool Matrix<T, Rows, Cols>::choleskyDecompose(Matrix& matrix, T* diag, T epsilon) {
    for (size_t i = 0; i < Rows; ++i) {
...
template <typename T, size_t Rows, size_t Cols>
void Matrix<T, Rows, Cols>::swapRows(Matrix& matrix, size_t row_a, size_t row_b) {
    for (size_t c = 0; c < Cols; ++c) {
...
template <typename T, size_t Rows, size_t Cols>
void Matrix<T, Rows, Cols>::subtractRow(Matrix& matrix, size_t target, size_t pivot, T factor) {
    for (size_t c = 0; c < Cols; ++c) {
